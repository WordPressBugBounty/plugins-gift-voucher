...
function wpgv__doajax_get_item_data() {
    ...
    $image_styles = array();
    for ($i = 0; $i < 3; $i++) {
        $style_image = esc_html(get_post_meta($item_id, 'style'.($i+1).'_image', true));
    ...
    echo wp_send_json(array(
        'title' => strip_tags(get_the_title($item_id)), 
        'images' => $image_styles, 
        'description' => html_entity_decode(strip_tags(esc_html(get_post_meta( $item_id, 'description', true )))), 
        'price' => esc_html(get_post_meta( $item_id, 'price', true )), 
        'special_price' => esc_html(get_post_meta( $item_id, 'special_price', true ))
    ));
    wp_die();
